...
def render_markdown_report(report: ClinicalReport) -> str:
    """Render a detailed, professional Markdown clinical report for human review."""
    lines: list[str] = []

    # ── Header ────────────────────────────────────────────────────────────────
    lines.append("# PharmaGenomics Advisor — Clinical Genomics Report")
    lines.append("")
    lines.append("> **DISCLAIMER:** This report is generated by an experimental AI research")
    lines.append("> prototype and is **not** a substitute for licensed clinical decision support.")
    lines.append("> All findings require review by a qualified clinical genomics professional")
    lines.append("> before any therapeutic action is taken.")
    lines.append("")

    # ── Report Metadata ───────────────────────────────────────────────────────
    lines.append("## Report Summary")
    lines.append("")
    generated_str = report.generated_at.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    ...
    lines.append(f"| Variants Classified | {len(report.classifications)} |")
    ...
    if not report.classifications:
        lines.append("_No routed variants were classified._")
    else:
        for i, item in enumerate(report.classifications, 1):
            acmg = item.classification.value if item.classification else "Unavailable"
            confidence = item.confidence.value if item.confidence else "Unknown"
            lines.append(f"### {i}. {item.gene} — {acmg} ({confidence} confidence)")
            lines.append("")
            lines.append(f"**Variant:** {item.variant_description}")
            lines.append("")
            lines.append("| Property | Value |")
            lines.append("|---|---|")
            lines.append(f"| Chromosome | {item.chromosome} |")
            lines.append(f"| Position | {item.position} |")
            lines.append(f"| Reference Allele | `{item.ref_allele}` |")
            lines.append(f"| Alternate Allele | `{item.alt_allele}` |")
            lines.append(f"| ACMG Classification | **{acmg}** |")
            lines.append(f"| Confidence | {confidence} |")
            if item.therapeutic_relevance:
                lines.append(f"| Therapeutic Relevance | {item.therapeutic_relevance.value} |")
            if item.functional_status:
                lines.append(f"| Functional Status | {item.functional_status.value} |")
            if item.data_sources_queried:
                lines.append(f"| Data Sources | {', '.join(item.data_sources_queried)} |")
            lines.append("")
            ...
            if item.clinical_narrative:
                ...
                lines.append("")
                lines.append(f"> {item.clinical_narrative}")
                lines.append("")
    ...
    lines.append("## Pharmacogenomic Drug Recommendations")
    lines.append("")
    if not report.drug_recommendations:
        lines.append("_No pharmacogenomic recommendations were generated._")
    else:
        lines.append("| Gene | Variant | Drug | Action | Evidence Level | Source |")
        lines.append("|---|---|---|---|---|---|")
        for rec in report.drug_recommendations:
            source_link = f"[Guidelines]({rec.guideline_source_url})" if rec.guideline_source_url else "N/A"
            lines.append(
                f"| {rec.gene} | {rec.variant} | **{rec.drug_name}** "
        ...
        if contra_items:
            ...
            for drug, gene, contra in contra_items:
                lines.append(f"- **{drug}** ({gene}): {contra}")
            lines.append("")
    ...
    if not report.literature_evidence:
        lines.append("_No literature evidence was retrieved._")
    else:
        for block in report.literature_evidence:
            ...
            if block.synthesis_paragraph:
                ...
                lines.append("")
            if block.citations:
                lines.append(f"**Supporting Citations ({len(block.citations)}):**")
                lines.append("")
                for cit in block.citations:
                    doi_link = f" DOI: [{cit.doi}](https://doi.org/{cit.doi})" if cit.doi else ""
                    lines.append(
                        f"- **{cit.title}** — {cit.authors} *{cit.journal}* ({cit.year})"
                        f"{doi_link}"
                    )
                    lines.append(f"  - {cit.evidence_summary}")
                lines.append("")
    ...
    if report.provenance:
        ...
        lines.append("")
        lines.append("| Agent | Data Sources | Confidence |")
        lines.append("|---|---|---|")
        for prov in report.provenance:
            sources = ", ".join(prov.data_sources_queried) if prov.data_sources_queried else "N/A"
            conf = prov.confidence.value if prov.confidence else "N/A"
            lines.append(f"| {prov.source_agent} | {sources} | {conf} |")
        lines.append("")
    ...
    if not report.warnings:
        lines.append("_No warnings were raised during pipeline execution._")
    else:
        for warning in report.warnings:
            if isinstance(warning, dict):
                ...
                location = f" `{variant}`" if variant else ""
                lines.append(f"- **[{stage}]**{location}: {message}")
            else:
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Generated by PharmaGenomics Advisor — Multi-Agent Precision Medicine Pipeline.*")
    lines.append("*This is a research prototype. Not for clinical use without professional oversight.*")
